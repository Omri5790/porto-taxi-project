...
// ──────────────────────────────────────────────────────── 11. the 20 km proof ──
{
  ...
  s.addText("סף 20 ק\"מ", he({ x: M, y: 0.7, w: W - 2 * M, h: 0.35,
    fontSize: 13, color: AMBER, bold: true }));
  s.addText("למה הוא ריק — והוכחה, לא הסבר", he({ x: M, y: 1.05, w: W - 2 * M, h: 0.7,
  ...
  s.addText("17,437 נסיעות כאלה קטן מספיק לבדיקה ממצה — הכל מול הכל, בסף של שתי נסיעות. זו הרצפה: אין \"משותף\" מתחת לשניים.",
    he({ x: M, y: 3.0, w: W - 2 * M, h: 0.6, fontSize: 16, color: "C9D6E5", lineSpacing: 24 }));

  const cards = [
    ["26.25", 'ק"מ — הקטע המשותף הארוך ביותר', "C9D6E5"],
    ["108", "מועמדים מעל 20 ק\"מ", "C9D6E5"],
    ["0", "מהם מסלול תקין", AMBER],
    ["19.91", 'ק"מ — המסלול האמיתי הארוך ביותר', AMBER],
  ];
